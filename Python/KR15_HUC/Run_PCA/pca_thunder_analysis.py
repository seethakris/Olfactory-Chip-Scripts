...
color_map, tsc,redo_pca,reconstruct_pca,stimulus_on_time, stimulus_off_time, color_mat,required_pcs, time_baseline):
    ...
    for ii in xrange(0, np.size(Stimulus_Directories, axis = 0)):
        ...
        for jj in xrange(0, np.size(Trial_Directories, axis = 0)):
            ...
            if len(txt_file)>0:
                #Load data        
                data_filtered = tsc.loadSeries(Working_Directory+name_for_saving_files+'_filtered.txt', inputFormat='text', nkeys=3).toTimeSeries().detrend(method='linear', order=15)
...
def run_analysis_allodor(Exp_Folder, filename_save_prefix_forPCA, filename_save_prefix_for_textfile, pca_components, num_pca_colors, num_samples, thresh_pca, color_map,\
 tsc,redo_pca,reconstruct_pca, stimulus_on_time, stimulus_off_time, color_mat, required_pcs, time_baseline):
    ...
    if len(txt_file)>0:
       #Load data                    
        data_filtered = tsc.loadSeries(Working_Directory+name_for_saving_files+'_filtered.txt', inputFormat='text', nkeys=3).toTimeSeries().detrend(method='linear', order=20)
        data_background = tsc.loadSeries(Working_Directory+name_for_saving_files+'.txt', inputFormat='text', nkeys=3)
