...
def make_kmeans_maps(data, kmeans_cluster_centers, img_labels, img_sim, img_size_x, img_size_y,ignore_clusters):
    ...
    interesting_clusters = np.array(np.where(np.logical_and(np.std(kmeans_cluster_centers,0)>0.0001,np.max(kmeans_cluster_centers,0)>0.00001)))
    print np.std(kmeans_cluster_centers,0)
    ...
    ### sort by maximum
    if ignore_clusters !=0:
        sort_kmeans = np.argsort(np.max(kmeans_cluster_centers_updated,0))[::-1]
        kmeans_cluster_centers_updated = kmeans_cluster_centers_updated[:,sort_kmeans]
    ...
    newclrs_updated_brewer =  update_colors(newclrs_brewer, ignore_clusters, interesting_clusters)
    
    #RGB colors
    newclrs_rgb = ListedColormap(sns.color_palette("bright", np.size(kmeans_cluster_centers,1)), name='from_list')
    newclrs_updated_rgb =  update_colors(newclrs_rgb, ignore_clusters, interesting_clusters)
...
def update_colors(newclrs,  ignore_clusters, interesting_clusters):
    ...
    if ignore_clusters !=0:
        ...
        newclrs_updated.colors[np.size(interesting_clusters):,:] = [0,0,0]
    else:
