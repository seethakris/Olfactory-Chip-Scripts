...
def make_pca_maps(pca, imgs, img_size_x, img_size_y,  num_pca_colors, num_samples, thresh_pca, color_map):
    
    maps = Colorize(totype=color_map, scale=num_pca_colors).images(imgs, )
    pts = pca.scores.subset(num_samples, thresh=thresh_pca, stat='norm')
    clrs = Colorize(totype=color_map, scale=num_pca_colors).points(pts[:,0:2])
