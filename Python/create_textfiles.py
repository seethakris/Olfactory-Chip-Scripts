...
time_start,time_end, f_f_flag, dff_start, dff_end,text_file):
    ...
    [data_mat, num_z_planes] = convert_tiff_to_mat(Working_Directory, img_size_x, img_size_y, img_size_crop_x, img_size_crop_y, text_file)    
    ...
    f_f_flag, dff_start, dff_end, stim_start, stim_end, smooth_window, pp)
    
    #Save as textfile
    pp.close()        
    print 'Saving all the data in the text file '            
    np.savetxt(Working_Directory+name_for_saving_files, numpy_array_for_thunder, fmt='%i')#Save as text file
...
time_start,time_end, f_f_flag, dff_start, dff_end, text_file):
    ...
    for jj in xrange(0, np.size(Trial_Directories, axis = 0)):
        ...
        #Send the file paths to get tiff files and convert them to mat and crop or resize appropriately
        [data_mat, num_z_planes] = convert_tiff_to_mat(Working_Directory1, img_size_x, img_size_y, img_size_crop_x, img_size_crop_y, text_file)    
        ...
        #Create proper matrix for textfile
        temp_numpy_array_for_thunder = get_matrix_for_textfile(data_mat, name_for_saving_figures1, z_planes, time_start, time_end, f_f_flag, dff_start, dff_end, stim_start, stim_end, smooth_window, pp)
...
def convert_tiff_to_mat(Working_Directory, img_size_x, img_size_y, img_size_crop_x, img_size_crop_y, text_file):
    ...
    for lst in xrange(1,np.size(onlyfiles, axis=0)+1):
        tif = TIFF.open(Working_Directory+'T='+str(lst)+'.tif', mode='r') #Open multitiff 
        ...
        if lst==1:
            ...
            for image in tif.iter_images():
                count_z = count_z + 1
            data = np.zeros((img_size_x-2*img_size_crop_x, img_size_y-2*img_size_crop_y, count_z,np.size(onlyfiles,0)), dtype=np.uint8)
        count_z = 0
        ...
        for image in tif.iter_images():
            ...
            if img_size_crop_x!= 0 and img_size_crop_y!=0:
                ...
                temp_image2 = temp_image1[img_size_crop_y:-img_size_crop_y, img_size_crop_x:-img_size_crop_x]
            elif img_size_crop_x==0 and img_size_crop_y!=0:
                if lst==1 and count_z==0:
                    print "Cropping only y pixels.."
                    text_file.write("Cropping only y pixels..\n")
                temp_image2 = temp_image1[img_size_crop_y:-img_size_crop_y, :]
            elif img_size_crop_x!=0 and img_size_crop_y==0:
                if lst==1 and count_z==0:                
                    print "Cropping only x pixels.."
                    text_file.write("Cropping only x pixels..\n")
                temp_image2 = temp_image1[:, img_size_crop_x:-img_size_crop_x]
            else:
...
def get_matrix_for_textfile(data_mat, name_for_saving_files, num_z_planes, time_start,time_end, f_f_flag, dff_start, dff_end, stim_start, stim_end, smooth_window, pp):
