...
def create_textfile_individual(Exp_Folder, filename_save_prefix, img_size_x, img_size_y,\
    ...
    np.savetxt(Exp_Folder+filesep+filename_save_prefix+'.txt', Matfile_for_Thunder, fmt='%i')#Save as text file
    
    return Matfile_for_Thunder
    
    
def get_data_from_tiff(tif, img_size_x, img_size_y, num_time, filename, pp):
...
def get_matrix_for_textfile(data, img_size_crop_x, img_size_crop_y, zz, time_start, time_end,f_f_flag, dff_start, dff_end,stim_start,stim_end,filename, pp):
    ...
    return temp_matfile_for_thunder[:,0:-smooth_window+2]
